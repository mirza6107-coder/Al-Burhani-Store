...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Inside burhani.js DOMContentLoaded
  const userName = document.querySelector(".signup-btn").innerText.trim();
  if (userName !== "SIGN IN" && !sessionStorage.getItem("welcomed")) {
  ...
  async function addToCart(product, btnElement) {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (badge) {
          badge.textContent =
            data.cart_count || parseInt(badge.textContent || 0) + product.qty;
        }
      } else {
